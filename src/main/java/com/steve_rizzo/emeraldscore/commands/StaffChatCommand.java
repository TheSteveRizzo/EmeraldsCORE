    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            if (p.hasPermission("emeraldsmc.staffchat")) {
                for (Player onlinePlayers : Main.core.getServer().getOnlinePlayers()) {
                    if (onlinePlayers.hasPermission("emeraldsmc.staffchat")) {
                        onlinePlayers.sendMessage(staffPrefix + ChatColor.translateAlternateColorCodes('&', message));
                        return true;
                    }
                }
                return true;
